...
계약은 추가만 했고 DB 마이그레이션이 없다. 버전이 어긋나도 영향은 새 API 두 개에 그치고, 공지 등록·QnA 답변·조회는 그대로 동작한다.

| notification | admin | 공지 수정·삭제 |
| --- | --- | --- |
| 구버전 | 신버전 | 500 `INTERNAL_SERVER_ERROR` (구버전이 `UNIMPLEMENTED` 반환) |
| 신버전 | 구버전 | API 없음. 새 RPC 는 호출되지 않는다 |
| 신버전 | 신버전 | 정상 |

- **기본 배포(`deploy.yml`)**: main 푸시 시 모든 서비스 이미지를 `:latest` 로 올리고, 호스트마다 `docker compose pull && docker compose up -d` 로 함께 재시작한다. admin 은 같은 호스트의 `notification:9090` 을 부르므로 재시작이 끝나면 호스트 안의 두 버전이 맞는다. 재시작 중 몇 초는 수정·삭제가 503(연결 실패) 또는 500(`UNIMPLEMENTED`)일 수 있다
- **서비스를 따로 배포할 때**: notification 을 모든 호스트에 올리고 헬스체크가 통과한 뒤 admin 을 올린다
- **서비스를 따로 롤백할 때**: admin 을 먼저 되돌리고 notification 을 되돌린다. 반대로 하면 그사이 수정·삭제가 500 이 된다
- **데이터**: 스키마 변경이 없어 롤백 때 되돌릴 마이그레이션이 없다. 이미 반영된 수정·삭제는 롤백해도 남는다 (삭제는 복구 불가)
- **트레이드오프**: 버전 협상이나 기능 플래그를 두지 않았다. 순서를 어겼을 때의 영향이 새 API 두 개의 일시적인 5xx 이고 기존 기능과 데이터에는 영향이 없어, 순서 규칙만으로 충분하다고 봤다
